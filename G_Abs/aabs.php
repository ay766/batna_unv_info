    ...
    $('#state').on('change', function(){
        ...
        if(country_idd){
            $.ajax({
                type:'POST',
                url:'ajaxData.php',
                data:'module='+country_idd,
                success:function(html){
                    $('#ens').html(html);
                }
            }); 
        }else{
            $('#ens').html('<option value="">Select promo first</option>'); 
        }
    });
     ...
     var etat = $('#etat').val();
     	...
     	if (annee_unive && country && state && ens && city && date && heurf && heurd && etat){

     		$.ajax({
                type:'POST',
                url:'ajaxData.php',
                data:{a:annee_unive,p:country,m:state,e:city,ens:ens,d:date,hd:heurd,hf:heurf,etat:etat},
                success:function(html){
                   	alert("cbon Absence ajouté");
		...
		<div id="fh5co-search-map">
			...
			<div class="search-property">
				<div class="s-holder">
					...
					<section>
								<label for="class">enseignants :</label>
								<select id="ens" name="ens">
									<option disabled selected value="">Select modules first</option>
							...
							<section>
								...
								<input id="heurd" min="08:30" max="17:00" value="08:30"  type="time" name="timed">
							</section>
							<section>
								<label for="class">heure_fin :</label>
								<input id="heurf" min="08:30" max="17:00" value="17:00" type="time" name="timef">
							</section>
<section>
								<label for="class">etat :</label>
								<select required="" id="etat" name="etat" >
									<option value="">choisis etat d'absence</option>
									<option value="J">justifié</option>
									<option value="N_J">Non_justifié</option>
						...
						<div class="col-xxs-12 col-xs-12 text-center">
							<input type="button" id="rech" name="submit" class="btn btn-primary btn-lg" value="Ajouté Absence" style="margin-bottom: 10px;" />
						</div>
						</form>
